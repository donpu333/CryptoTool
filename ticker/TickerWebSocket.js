class TickerWebSocket {
   ...
   setupWebSockets() {
    ...
    // НЕ переподключаем, если вкладка скрыта
    if (document.hidden) {
        console.log(`⏭️ Вкладка скрыта, не переподключаем Binance ${marketType}`);
        return;
    }
    
    // НЕ переподключаем, если нет символов для этого рынка
    if (!this.parent.tickers.some(t => t.exchange === 'binance' && t.marketType === marketType)) {
        console.log(`⏭️ Нет символов для Binance ${marketType}, не переподключаем`);
        return;
    }
    ...
    if (this.parent.state.wsReconnectAttempts[attemptKey] < this.parent.state.maxReconnectAttempts) {
        ...
        console.log(`🔄 Переподключение Binance ${marketType} через ${delay/1000}с`);
        const timer = setTimeout(() => {
            this.connectBinanceWebSocket(marketType, url);
        }, delay);
        this.wsReconnectTimers.push(timer);
    }
};
    }
...
cleanup() {
    ...
    for (const ws of this.wsConnections) {
        // 👇 НЕ ТРОГАЕМ kline WebSocketManager
        if (ws === window.wsManager?.wsKline || ws === window.wsManager?.wsKlineBybit) {
            continue;
        }
