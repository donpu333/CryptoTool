...
setDataQuick(data, interval, symbol, exchange = 'binance', marketType = 'futures') {
    ...
    if (data.length > 0) {
        ...
        // ВАЖНО: Binance API возвращает время в миллисекундах, делим на 1000
        // и выравниваем по началу интервала
        data = data.map(c => ({
            ...c,
...
updateCurrentCandle(price) {
  ...
  updateRealPrice(price) {
    this.currentRealPrice = price;
    // НЕ вызываем updateCurrentCandle
...
restoreIndicators() {
 ...
 _subscribeToPrice() {
    ...
    // Создаём новый обработчик, который **НЕ ЛОМАЕТ** свечи
    this._priceUpdateHandler = (price, symbol) => {
        if (symbol === this.currentSymbol && document.hidden) {
            // Обновляем цену ТОЛЬКО когда вкладка скрыта (восстановление)
            this.currentRealPrice = price;
        }
        // НЕ обновляем свечи, это делает WebSocketManager
    };
